...
router.post('/register', async (req, res) => {
  ...
  if (error.error) return res.status(411).json(error.error.details[0].message)
...
router.post('/login', async (req, res) => {
  ...
  if (error.error) return res.status(411).send(error.error.details[0].message)

  // Check if the user exist in db
  const user = await User.findOne({ email: req.body.email })
  if (!user) return res.status(409).send('Email or password is wrong!')

  // Check the password is correct
  const vaildPassword = await bcrypt.compare(req.body.password, user.password)
  if (!vaildPassword) return res.status(409).send('Email or password is wrong! Please try again.')
